addEventListener('DOMContentLoaded', () => {
    ...
    const sortbaleContainer = document.getElementById("upper__tasks__block__list");
    const sortable = new Sortable(sortbaleContainer, {
        draggable: sortableElementsSelector,
        onEnd: function () {
            console.log('drag:stop')

            document.querySelectorAll('.sortable-mirror').forEach(e => e.remove());

            const waitTasksContainer = document.getElementById("upper__tasks__block__list");
            const waitTasks = waitTasksContainer.querySelectorAll(".waiting-task");
    ...
    function startTimer(timerDiv) {
        setInterval(function () { incrementTimer(timerDiv); }, 1000);
    }
    ...
    function incrementTimer(timerDiv) {
        ...
        if (seconds >= 60) {
            ...
            let innerDivTextContent = activeTask.querySelector(".list__item__strend").textContent;
            ...
            let endTimeFinal = (endHours < 10 ? "0" + endHours : endHours) + ":" +
    ...
    function calculateTimeDifference(e) {
        ...
        if (e.target.getAttribute('name') == "task__start") {
            ...
            startTimeInput = e.target.previousElementSibling.previousElementSibling
        ...
        if ((timeFormat.test(startTimeInput.value) && timeFormat.test(endTimeInput.value)) === false) {
...
function sortAndGetTotalTime() {
    ...
    let sortedTasks = Array.from(tasksWithTime).sort((a, b) => {
        ...
        let timeA = a.querySelector('.list__item__strend').textContent.trim().split(':');
        let timeB = b.querySelector('.list__item__strend').textContent.trim().split(':');
    ...
    tasksWithTime.forEach(task => {
        ...
        let duration = task.querySelector('.list__item__sepndtime').textContent.trim();
        ...
        if (timeSplitted.length == 2) {
            ...
            totalDuration += parseInt(timeSplitted[0]) * 60 + parseInt(timeSplitted[2]);
        }
